...
namespace Exercise_02_UpgradedPhoneBook
{
    public class updatedPhoneBook
    {
        ...
        static void ListAll(Dictionary<string, string> phoneBook)
        {
            SortedDictionary<string, string> sortedPhoneBook = new SortedDictionary<string, string>();

            foreach (var entry in phoneBook)
            {
                sortedPhoneBook.Add(entry.Key, entry.Value);
            }

            foreach (var sortedEntry in sortedPhoneBook)
            {
                Console.WriteLine(sortedEntry.Key + " -> " + sortedEntry.Value);
            }
        }
    }
}
